...
This `v2-template-shooterGame` copy was developed against a sibling local SDK
checkout rather than only the production tarball. Keep this note local to this
project copy; do not change the original upstream template grounding for this
project-specific workflow.

Use the published SDK package for normal development:

```sh
cd v2-template-shooterGame
npm install
npm run dev
```

The app code should import SDK APIs normally:
`import { ... } from "@vincentt-xr/sdk"`.

Use `<ScreenText>` from the SDK for future end-user screen copy. Do not add
creator/debug ScreenText settings panels to this template; finished projects
should ship only the authored scene UI.
